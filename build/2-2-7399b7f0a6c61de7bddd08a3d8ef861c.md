...
- [Redis Connect](https://github.com/redis-developer/redis-connect): Explicitly segregates Job Management (control plane) from Job Execution (data plane). The control plane handles job orchestration, metrics reporting, and cluster management through a REST API and CLI.
- Kubernetes Operators: Solutions like the [Kuaishou solution](https://github.com/apecloud/kubeblocks) with KubeBlocks use a centralized control plane for cross-cluster orchestration, resource distribution, and status aggregation across multiple member clusters.
- [Redis Operator](https://github.com/spotahome/redis-operator) (CloudNativePG style): Uses a Controller Manager in the control plane to reconcile cluster resources while the Instance Manager handles local processes.
